# SMM Enhanced Attestation (SEA) Overview

This document provides an overview of the SMM Enhanced Attestation (SEA) feature. To understand why SEA exists and what
it does, the following topics will be reviewed.
...
Key system resources in x86 architecture include IO port accesses, Model Specific Register (MSR) accesses, MMIO access,
CPU save state access and allowed instructions in the *User Mode* MM environment. The code that runs in
*Supervisor Mode* in this repository, is called the "MM Supervisor". The configuration of allowed resources is defined
by a "MM Supervisor Policy". This policy is used to configure the exact details of what hardware resources to restrict,
...
The firmware ecossytem contains hundreds of drivers written by different vendors. Manufacturers also differentiate
...
[UEFI](https://uefi.org/) firmware code. The [MM Supervisor feature](https://github.com/microsoft/mu_feature_mm_supv)
...
and later, Intel Trusted Execution Technology (TXT) is more widely available across processor SKUs. It was previously
restricted to Intel vPro processors. Broader support enables a dynamic launch to become standard across Intel devices
which can support the SEA feature.
